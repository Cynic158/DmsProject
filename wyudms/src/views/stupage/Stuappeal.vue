<template>
  <div style="position: relative">
      ...
      <el-table-column align="center" prop="id" width="180">
        ...
        <template #default="scope">
          <el-button
            ...
            size="small"
            type="primary"
            @click="handleAppealEdit(scope.$index, scope.row)"
            >更改</el-button
          >
          <el-button
            style="
    ...
    <button class="clearAppeal" @click="sendappeal">发布诉求</button>
  ...
  ></Dmsconfirm>
  <Stuformappeal @settable="settable"></Stuformappeal>
</template>

<script>
import Dmsconfirm from "@/components/Dmsconfirm.vue";
import { getAppealListbyid, getAppealinfobyid } from "@/api/appeal";
import { onMounted, reactive, ref } from "vue";
import { useStore } from "vuex";
import Stuformappeal from "@/components/Stuformappeal.vue";
export default {
  name: "stuappeal",
  components: {
    Dmsconfirm,
  ...
  setup() {
    ...
    const setappealtable = (val) => {
      getAppealListbyid({ page: val, stuid: store.state.dsuser.dsuserInfo.id })
        .then((res) => {
          if (res.status == 0) {
    ...
    const setappealinfo = (val) => {
      getAppealinfobyid({ id: val, stuid: store.state.dsuser.dsuserInfo.id })
        .then((res) => {
          if (res.status == 0) {
    ...
    const handleAppealEdit = (index, row) => {
      // console.log(index, row);
      store.commit("dsappeal/setAppealFormId", row.id);
      store.commit("dsappeal/setAppealFormType", 1);
      store.commit("dsappeal/setAppealFormStatus");
    };
    ...
    const sendappeal = () => {
      // console.log("发送诉求");
      store.commit("dsappeal/setAppealFormType", 0);
      store.commit("dsappeal/setAppealFormStatus");
    };
