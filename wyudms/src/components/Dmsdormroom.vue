<template>
  <Transition name="room">
    <div class="dormroommask" v-show="dormroomshow">
...
export default {
  ...
  setup() {
    ...
    const dormroomshow = ref(false);

    const dormroomhide = () => {
      dormroomshow.value = false;
    };
    ...
    onMounted(() => {
      watch(
        () => store.state.dsdorm.dormroomstatus,
        () => {
          roomstu.stulist = store.state.dsdorm.roomstu;
          roomstu.roomname = store.state.dsdorm.roomname;
          dormroomshow.value = true;
        }
      );
    });
    return { dormroomshow, dormroomhide, roomstu };
  },
};
</script>
