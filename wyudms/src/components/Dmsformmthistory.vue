...
import { getmaintainbyid } from "@/api/maintain";
...
export default {
  ...
  setup() {
    ...
    const settable = () => {
      mtformhide.value = false;
      store.commit("dsmaintain/setMtHistoryTableStatus");
    };
    ...
    onMounted(() => {
      ...
      watch(
        () => store.state.dsmaintain.editmthistorystatus,
        () => {
          getmaintainbyid({ id: store.state.dsmaintain.editmtid })
            .then((res) => {
              if (res.status == 0) {
                ...
                if (res.maintaininfo[0].status == 2) {
                  res.maintaininfo[0].status = "已处理";
                } else if (res.maintaininfo[0].status == 3) {
                  res.maintaininfo[0].status = "已取消";
                }
